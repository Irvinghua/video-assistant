...
export class YouTubeService implements IPlatformService {
    ...
    async getComments(videoId: string, limit: number = 100): Promise<Comment[]> {
        ...
        try {
            const comms = await getYouTubeComments(videoId, limit);
            this.log(`Successfully got ${comms?.length || 0} comments`);
            return comms || [];
        } catch (e) {
    ...
    async getAudioUrl(_videoId: string): Promise<string | null> {
        return null;
    }
